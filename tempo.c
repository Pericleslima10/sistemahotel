...
{
    struct tm d1_tm = {0}, d2_tm = {0}; // Inicializa as estruturas com zeros

    // Convertendo d2 para struct tm
    d2_tm.tm_hour = d2.hora;
    d2_tm.tm_min = d2.minuto;
    d2_tm.tm_sec = d2.segundo;
    d2_tm.tm_year = d2.ano - 1900; // Ajuste para a base correta (anos desde 1900)
    d2_tm.tm_mon = d2.mes - 1;     // Ajuste para a base correta (meses desde janeiro = 0)
    d2_tm.tm_mday = d2.dia;
    d2_tm.tm_isdst = -1; // Deixa o sistema determinar o DST

    // Convertendo d1 para struct tm
    d1_tm.tm_hour = d1.hora;
    d1_tm.tm_min = d1.minuto;
    d1_tm.tm_sec = d1.segundo;
    d1_tm.tm_year = d1.ano - 1900; // Ajuste para a base correta
    d1_tm.tm_mon = d1.mes - 1;     // Ajuste para a base correta
    d1_tm.tm_mday = d1.dia;
    d1_tm.tm_isdst = -1; // Deixa o sistema determinar o DST

    // Convertendo estruturas tm para time_t
    time_t d1_t = mktime(&d1_tm);
    time_t d2_t = mktime(&d2_tm);

    // Calculando a diferença em segundos e convertendo para dias
    double seconds = difftime(d2_t, d1_t);
    int dias = (int)(seconds / 86400); // Divide diretamente por 86400 segundos/dia
